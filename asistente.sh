...
echo 'Antes de comenzar, asegurate de que este directorio y todo su contenido estan en la ruta /root/podman y de haber creado en podman una red llamada "proyecto"'
echo 'puedes crear dicha red lanzando "podman network create proyecto"'
...
if [ "$(pwd)" != "/root/podman/proyecto" ]; then
...
sleep 1
podman kube play --network proyecto stack_ldap/tmp.yml
...
podman exec proyecto_stackldap-openldap ldapmodify -Y EXTERNAL -H ldapi:/// -f /var/lib/ldap/load_memberof.ldif
podman exec proyecto_stackldap-openldap ldapmodify -Y EXTERNAL -H ldapi:/// -f /var/lib/ldap/add_memberof_overlay.ldif
...
sleep 1
podman kube play --network proyecto stack_nextcloud/tmp.yml
...
podman kube play --network proyecto stack_grafana/stack_grafana.yml

#Despliegue del proxy
echo 'Desplegando proxy reverso Caddy Server...'

podman run -d \
  --name=proyecto_caddyproxy \
  --network=proyecto \
  -p 80:80 \
  -p 443:443 \
  -v /root/podman/proyecto/caddy_proxy/volumes/Caddyfile:/etc/caddy/Caddyfile:z \
  -v /root/podman/proyecto/caddy_proxy/volumes/data:/data:z \
  -v /root/podman/proyecto/caddy_proxy/volumes/config:/config:z \
  caddy:latest

echo "Reiniciando proxy inverso..."
sleep 2
podman container restart proyecto_caddyproxy
...
clear
	...
	# Redespliegue
            echo "Ejecutando comandos de redespliegue."
                ...
                podman kube play --network proyecto ./stack_ldap/redeploy.yml
                echo 'Desplegando Nextcloud y base de datos'
                podman kube play --network proyecto ./stack_nextcloud/redeploy.yml
                echo 'Desplegando Grafana y Prometheus' 
                podman kube play --network proyecto ./stack_grafana/redeploy.yml
...
echo
		...
		read
            ...
            podman pod start proyecto_stacknc proyecto_stackldap proyecto_stackgrafana && podman container start proyecto_caddyproxy
echo
...
echo
		...
		read
            ...
            podman pod stop proyecto_stacknc proyecto_stackldap proyecto_stackgrafana && podman container stop proyecto_caddyproxy
echo
...
echo
            ...
            podman pod restart proyecto_stacknc proyecto_stackldap proyecto_stackgrafana && podman container restart proyecto_caddyproxy
echo
...
clear 
	    ...
	    podman pod stop proyecto_stacknc proyecto_stackldap proyecto_stackgrafana && podman container stop proyecto_caddyproxy
	    podman pod rm proyecto_stacknc proyecto_stackldap proyecto_stackgrafana && podman container rm proyecto_caddyproxy
echo
...
clear 
	...
	echo "y añadelo al directorio /root/podman/proyecto/stack_ldap"
echo
	echo "PULSA INTRO PARA CONTINUAR..."
	read
...
echo
            ...
            podman pod stop proyecto_stackldap proyecto_stacknc proyecto_stackgrafana && podman container stop proyecto_caddyproxy
	    podman pod rm proyecto_stackldap proyecto_stacknc proyecto_stackgrafana && podman container rm proyecto_caddyproxy
...
cat <<EOF
 ...
 - Si deseas realizar una copia de toda la instancia para su despliegue en otra maquina o una restauracion en la misma, debes restaurar el directorio /root/podman/proyecto en la maquina objetivo y elegir
